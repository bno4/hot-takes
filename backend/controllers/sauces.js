  ...
  Sauce.findOne({ _id: req.params.id })
    .then((sauce) => {
      ...
      // si l'utilisateur est à l'origine du post sauce, update de la sauce
        ...
        // si non présence d'un fichier update
        else {
          Sauce.updateOne(
            { _id: req.params.id },
            { ...sauceObject, _id: req.params.id }
...
exports.rateSauce = (req, res, next) => {
  ...
  if (like === 1) {
    ...
    Sauce.updateOne(
      { _id: chosenSauce },
      { $inc: { likes: 1 }, $push: { usersLiked: clientId }, _id: chosenSauce }
    )
        ...
        if (sauce.usersLiked.indexOf(clientId) !== -1) {
          Sauce.updateOne(
            ...
            .then(() =>
              res
                .status(200)
                .json({ message: "sauce modifiée, suppression du like" })
            )
            .catch((error) => res.status(400).json({ error }));
          ...
          Sauce.updateOne(
            ...
            .then(() =>
              res
                .status(200)
                .json({ message: "sauce modifiée, suppression du unlike" })
            )
            .catch((error) => res.status(400).json({ error }));
        }
      })
